...
public class SvdMemoryMapUpdateTask extends Task {
	...
	public void run(TaskMonitor monitor) throws CancelledException {
		...
		// Apply all of the accepted operations inside a single transaction...
		int transactionId = mProgram.startTransaction("Load SVD memory map");
		boolean ok = false;
		try {
	...
	private void applyMemoryCreateOperation(MemoryBlockOperation op) {
		...
		} catch (LockException | MemoryConflictException | IllegalArgumentException
				| AddressOverflowException e) {
			Msg.error(this, "Could not create memory block " + op.Name + " at " + addr, e);
		}
	}
	...
	private void applyMemoryUpdateOperation(MemoryBlockOperation op) {
		try {
			...
			Msg.error(this, "Could not update memory block " + op.Name, e);
		}
	}
}